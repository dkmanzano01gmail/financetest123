          ...
          <Card className="border-dashed">
            ...
            <CardContent className="space-y-4 text-xs">
              ...
              <CalculationSection title="3. Queima biscoito">
                <FiringBreakdown label="Biscoito" profile={calculation.bisqueProfile} usePercent={calculation.kilnUsePercent} resistanceCost={calculation.bisqueResistanceCost} baseAdded={calculation.bisqueResistanceBaseAdded} internalCost={calculation.bisqueInternalCost} billingCost={calculation.bisqueBillingCost} enabled={form.charge_biscuit} resistanceOnly={calculation.resistanceOnly} profitRate={calculation.kilnProfit} currency={currency} privacy={privacy} configured={!!selectedKiln} settingsSaved={calculation.settingsSaved} lengthCm={calculation.effectiveLengthCm} depthCm={calculation.effectiveDepthCm} heightCm={calculation.effectiveHeightCm} firingIndex={calculation.bisqueFiringIndex} />
              </CalculationSection>

              <CalculationSection title={`4. Queima de esmalte — cone ${form.glaze_cone || "6"}`}>
                <FiringBreakdown label="Esmalte" profile={calculation.glazeProfile} usePercent={calculation.kilnUsePercent} resistanceCost={calculation.glazeResistanceCost} baseAdded={calculation.glazeResistanceBaseAdded} internalCost={calculation.glazeInternalCost} billingCost={calculation.glazeBillingCost} enabled={form.charge_glaze} resistanceOnly={calculation.resistanceOnly} profitRate={calculation.kilnProfit} currency={currency} privacy={privacy} configured={!!selectedKiln} settingsSaved={calculation.settingsSaved} lengthCm={calculation.effectiveLengthCm} depthCm={calculation.effectiveDepthCm} heightCm={calculation.effectiveHeightCm} firingIndex={calculation.glazeFiringIndex} />
              </CalculationSection>
...
function FiringBreakdown({ label, profile, usePercent, resistanceCost, baseAdded, internalCost, billingCost, enabled, resistanceOnly, profitRate, currency, privacy, configured, settingsSaved, lengthCm, depthCm, heightCm, firingIndex }: { label: string; profile: any; usePercent: number; resistanceCost: number; baseAdded: number; internalCost: number; billingCost: number; enabled: boolean; resistanceOnly: boolean; profitRate: number; currency: string; privacy: boolean; configured: boolean; settingsSaved: boolean; lengthCm: number; depthCm: number; heightCm: number; firingIndex: number }) {
  ...
  return <>
    <CalculationLine title="Resistência rateada para a peça" formula={`${formatCurrency(profile.resistanceCost, currency, privacy)} de resistência total ÷ ${profile.resistanceBurns.toLocaleString("pt-BR")} queimas estimadas × ${(usePercent * 100).toFixed(3)}% de ocupação = ${formatCurrency(resistanceCost, currency, privacy)}`} sources={source} />
    {baseAdded > 0 && <CalculationLine title="Complemento mínimo configurado" formula={`${formatCurrency(resistanceCost, currency, privacy)} + ${formatCurrency(baseAdded, currency, privacy)} = ${formatCurrency(internalCost, currency, privacy)}`} sources={<SourceTag kind={settingsSaved ? "config" : "assumed"}>Complemento aplicado pelo ateliê</SourceTag>} />}

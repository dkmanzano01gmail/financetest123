...
export const Route = createFileRoute("/_authenticated/atelier/class-materials")({
  component: Page,
});
...
function Page() {
  ...
  const totals = useMemo(() => {
    ...
    for (const r of filtered) {
      total += Number(r.amount_charged);
      if (r.payment_status === "paid") paid += Number(r.amount_charged);
      else if (r.payment_status === "pending") pending += Number(r.amount_charged);
    }
  ...
  const save = useMutation({
    mutationFn: async () => {
      const p: any = {
        ...
        material: f.material,
        grams: Number(f.grams.replace(",", ".") || 0),
        amount_charged: Number(f.amount_charged.replace(",", ".") || 0),
        payment_status: f.payment_status,
        payment_date: f.payment_date || null,
        comments: f.comments || null,
      };
      const { error } = editId
        ? await sb.from("class_materials_usage").update(p).eq("id", editId).eq("workspace_id", wsId)
        : await sb.from("class_materials_usage").insert(p);
  ...
  return (
    <PageContainer>
      <PageHeader
        ...
        description="Uso de material por aluno e pagamento"
        action={
          <Button
            onClick={() => {
              setEditId(null);
        ...
        <Card>
          <CardContent className="p-4">
            ...
            <div className="font-mono text-xl">
              {formatCurrency(totals.total, currency, privacy)}
            </div>
          </CardContent>
        </Card>
        <Card>
          <CardContent className="p-4">
            ...
            <div className="font-mono text-xl text-income">
              {formatCurrency(totals.paid, currency, privacy)}
            </div>
          </CardContent>
        </Card>
        <Card>
          <CardContent className="p-4">
            ...
            <div className="font-mono text-xl text-expense">
              {formatCurrency(totals.pending, currency, privacy)}
            </div>
          </CardContent>
        </Card>
      </div>
      ...
      <div className="flex gap-2 mb-3">
        <Input
          placeholder="Buscar aluno/material"
          value={q}
        ...
        <Select value={status} onValueChange={setStatus}>
          ...
          <SelectContent>
            ...
            <SelectItem value="waived">Cortesia</SelectItem>
          </SelectContent>
        </Select>
      </div>
            ...
            <table className="w-full text-sm">
              <thead className="bg-muted/40">
                <tr className="text-left">
                  ...
                  <th className="p-3 text-right">Gramas</th>
              ...
              <tbody>
                {filtered.map((r: any) => (
                  <tr key={r.id} className="border-t border-border">
                    <td className="p-3 font-mono">{r.usage_date}</td>
                    ...
                    <td className="p-3 text-right font-mono">
                      {formatCurrency(Number(r.amount_charged), currency, privacy)}
                    </td>
          ...
          <div className="grid grid-cols-2 gap-3">
            <div className="space-y-1.5">
              ...
              <Input
                type="date"
                value={f.usage_date}
                onChange={(e) => setF({ ...f, usage_date: e.target.value })}
              />
            </div>
            <div className="space-y-1.5">
              <Label>Aluno</Label>
            ...
            <div className="space-y-1.5">
              <Label>Gramas</Label>
              <Input value={f.grams} onChange={(e) => setF({ ...f, grams: e.target.value })} />
            </div>
            <div className="space-y-1.5">
              <Label>Valor cobrado</Label>
              <Input
                value={f.amount_charged}
                onChange={(e) => setF({ ...f, amount_charged: e.target.value })}
              />
            </div>
            <div className="space-y-1.5">
              <Label>Status</Label>
